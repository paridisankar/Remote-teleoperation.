...
class Teleop():
    ...
    def mqtt_connect(self):
        ...
        self.mqtt_client = AWSIoTMQTTClient(rospy.param("robot_name").lower())
        self.mqtt_client.configureEndpoint(rospy.param("iot_endpoint").lower(), int(rospy.param("mqtt_port")))
    ...
    def custom_callback(self, client, userdata, message):
        ...
        """
        logger.info('Received from %s, message: %s', rospy.param("robot_name").lower(), message.payload)
    ...
    def subscribe_joystick(self):
        ...
        """
        logger.info("Subsribing to topic %s", rospy.param("robot_name").lower())
        self.mqtt_client.subscribe(rospy.param("robot_name").lower(), 1, self.custom_callback)
        return
